...
eval "\"${INIMerger}\" -tron \"${Output}/edit_all_flags.ini\" edit_undestroyable_flags.ini edit_advanced_flags.ini edit_flags.ini"
#eval "\"${INIMerger}\" -tron \"${Output}/[8] Normandy clean.map\" \"${Output}/[8] Normandy clean.map\" \"${Output}/edit_all_flags.ini\""
#eval "\"${INIMerger}\" -tron \"${Output}/[8] Normandy clean.map\" \"${Output}/[8] Normandy clean.map\" \"edit_proxy_mcv.ini\""

eval "\"${INIMerger}\" -tron \"${Output}/gm_invasion_vanilla.ini\" edit_normandy_control_points.ini"

eval "\"${INIMerger}\" -tron \"${Output}/gm_invasion.ini\" edit_proxy_mcv.ini edit_undestroyable_flags.ini edit_advanced_flags.ini edit_flags.ini fix_bfrt.ini fix_grand_cannon.ini fix_siege_chopper.ini edit_repairable_bunkers.ini edit_paradrop.ini edit_transport_zeppelin_for_scripts.ini edit_capturable_lax.ini edit_secret_lab.ini edit_outpust_allow_mcv_conopt.ini edit_ext_occupy.ini edit_transport_via_factory.ini edit_c4_tech.ini edit_normandy_control_points.ini buff_paradrop.ini buff_tesla_coil.ini buff_v3.ini buff_transport.ini buff_bunkers.ini"
